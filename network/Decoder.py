...
class Decoder_512(nn.Module):
    def __init__(self, latent_dim, hidden_dims=[16, 32, 64, 128, 256, 512, 1024, 2048]): # hidden_dims=[32, 64, 128, 256, 512, 1024, 2048]/[32, 64, 64, 128, 128, 256, 256, 512]
        ...
        self.output_layer = nn.Sequential(
            nn.ConvTranspose2d(in_channels=hidden_dims[-1], out_channels=8, kernel_size=3, stride=2, padding=1, output_padding=1, bias=False),
            nn.InstanceNorm2d(num_features=8, affine=True),
            nn.LeakyReLU(inplace=True),

            nn.ConvTranspose2d(in_channels=8, out_channels=3, kernel_size=3, stride=2, padding=1, output_padding=1, bias=False),
            nn.InstanceNorm2d(num_features=3, affine=True),
            nn.LeakyReLU(inplace=True),
    ...
    def forward(self, latent_z):
        ...
        result = self.output_layer(result) # bs*16*128*128 -> bs*3*512*512

        return result



class Decoder_1024(nn.Module):
    def __init__(self, latent_dim, hidden_dims=[16, 32, 64, 128, 256, 512, 1024, 2048]): # hidden_dims=[32, 64, 128, 256, 512, 1024, 2048]/[32, 64, 64, 128, 128, 256, 256, 512]
        super(Decoder_1024, self).__init__()
        ...
        self.output_layer = nn.Sequential(
            ...
            nn.ConvTranspose2d(in_channels=8, out_channels=8, kernel_size=3, stride=2, padding=1, output_padding=1, bias=False),
            nn.InstanceNorm2d(num_features=8, affine=True),
            nn.LeakyReLU(inplace=True),
    ...
    def forward(self, latent_z):
        ...
        result = self.output_layer(result) # bs*16*128*128 -> bs*3*1024*1024
