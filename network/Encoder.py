...
class PsprpEncoder(nn.Module):
    def __init__(self, num_layers, mode='ir'):
        super(PsprpEncoder, self).__init__()
        assert num_layers in [50, 100, 152], 'num_layers should be 50,100, or 152'
        assert mode in ['ir', 'ir_se'], 'mode should be ir or ir_se'
        blocks = get_blocks(num_layers)
        if mode == 'ir':
        ...
        self.output_layer = nn.Sequential(
            ...
            nn.Flatten(),
            #nn.Linear(in_features=512 * 7 * 7, out_features=512),
            nn.Linear(in_features=512 * 16 * 16, out_features=2048),
        )

        self.fc_mu = nn.Linear(in_features=2048, out_features=512)
        self.fc_var = nn.Linear(in_features=2048, out_features=512)
    ...
    def forward(self, x):
        x = self.input_layer(x)
        x = self.body(x) # bsx512x16x16
        x = self.output_layer(x) # bsx2048 / bsx512

        mu = self.fc_mu(x)
        log_var = self.fc_var(x)
        feat = self.reparameterize(mu=mu, logvar=log_var)

        return mu, log_var, feat
