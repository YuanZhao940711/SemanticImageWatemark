...
class Train:
    def __init__(self, args):
        ...
        decoder_statedict = self.decoder.state_dict()
        decoder_ckpt = torch.load(os.path.join(self.args.checkpoint_dir, 'stylegan2-ffhq-config-f.pt'), map_location=self.args.device)
        decoder_ckpt = {k: v for k, v in decoder_ckpt.items() if k in decoder_statedict}
        decoder_statedict.update(decoder_ckpt)
        self.decoder.load_state_dict(decoder_statedict, strict=True)
