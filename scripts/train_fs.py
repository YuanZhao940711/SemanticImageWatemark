...
class Train:
    def __init__(self, args):
        ...
        try:
            ...
            print_log("[*]Training Disentangler Backbone from scratch", self.args.logpath)
            self.disbackbone.apply(weights_init)
        
        # Disentangler Id Encoder
        ...
        try:
            ...
            print_log("[*]Training Disentangler IdEncoder from scratch", self.args.logpath)
            self.disidencoder.apply(weights_init)
        
        # Disentangler Att Encoder
        ...
        #self.dis_optim = optim.Adam(self.disentangler.paramerters(), lr=self.args.dis_lr, betas=(0.9, 0.999))
        param = list(list(self.disbackbone.parameters()) + list(self.disidencoder.parameters()) + list(self.disattencoder.parameters()))
        self.dis_optim = optim.Adam(param, lr=self.args.dis_lr, betas=(0.9, 0.999))
    ...
    def forward_pass(self, image_ori):
        ...
        image_ori_feats = self.disbackbone(image_ori)
        image_ori_id = self.disidencoder(image_ori_feats)
        image_ori_id_norm = l2_norm(image_ori_id)
        image_ori_att = self.disattencoder(image_ori_feats)
        ...
        image_rec_feats = self.disbackbone(image_rec)
        image_rec_id = self.disidencoder(image_rec_feats)
        image_rec_id_norm = l2_norm(image_rec_id)
        image_rec_att = self.disattencoder(image_rec_feats)
    ...
    def training(self, epoch, image_loader):
        ...
        self.disbackbone.train()
        self.disidencoder.train()
        self.disattencoder.train()
        ...
        """
        train_epoch_log = "Training[{:d}/{:d}] Att_loss: {:.6f} Id_loss: {:.6f} Rec_loss: {:.6f} Sumlosses={:.6f} BatchTime: {:.4f}".format(
            epoch+1, self.args.max_epoch, Att_loss.avg, Id_loss.avg, Rec_loss.avg, Train_losses.avg, batch_time.sum
        )
    ...
    def validation(self, epoch, image_loader):
        ...
        """
        val_log = "Validation[{:d}] Att_loss: {:.6f} Id_loss: {:.6f} Rec_loss: {:.6f} Kl_loss: {:.6f} Sumlosses={:.6f} BatchTime: {:.4f}".format(
        ...
        for epoch in range(self.args.max_epoch):
            ...
            if (epoch+1) % self.args.validation_interval == 0:
                ...
                stat_dict = {
                    ...
                    'disbackbone_state_dict': self.disbackbone.state_dict(),
                    'disidencoder_state_dict': self.disidencoder.state_dict(),
    ...
    def save_checkpoint(self, state, is_best):
        if is_best:
            ...
            torch.save(state['disbackbone_state_dict'], os.path.join(self.args.bestresults_dir, 'checkpoints', 'DisBackbone_best.pth'))
            torch.save(state['disidencoder_state_dict'], os.path.join(self.args.bestresults_dir, 'checkpoints', 'DisIdEncoder_best.pth'))
