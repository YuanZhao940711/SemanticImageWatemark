...
from utils.common import visualize_vae_results, print_log, l2_norm, log_metrics, AverageMeter
...
class Train:
    def __init__(self, args):
        ...
        train_dataset = ImageDataset(root=self.args.train_cover_dir, transforms=train_transforms)
        val_dataset = ImageDataset(root=self.args.val_cover_dir, transforms=val_transforms)
    ...
    def training(self, epoch, train_loader):
        ...
        for train_iter, image_batch in enumerate(train_loader):

            image_batch = image_batch.to(self.args.device)
            ...
            if self.global_train_steps == 0 or (self.global_train_steps+1) % self.args.image_interval == 0:
                visualize_vae_results(vis_dict=data_dict, dis_num=self.args.display_num, epoch=epoch+1, prefix='train', save_dir=self.args.trainpics_dir, iter=train_iter+1, step=self.global_train_steps+1)
    ...
    def validation(self, epoch, val_loader):
        ...
        for val_iter, image_batch in enumerate(val_loader):
        ...
        visualize_vae_results(vis_dict=data_dict, dis_num=self.args.display_num, epoch=epoch+1, prefix='validation', save_dir=self.args.valpics_dir)
        ...
        for epoch in range(self.args.max_epoch):
            ...
            if epoch % self.args.validation_interval == 0:
                ...
                if (self.best_loss is None) or (validation_loss < self.best_loss):
                    ...
                    visualize_vae_results(vis_dict=data_dict, dis_num=self.args.display_num, epoch=epoch, prefix='best', save_dir=self.args.bestresults_dir)
